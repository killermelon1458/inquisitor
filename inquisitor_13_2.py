...
from inquisitor_perms_2 import PermissionManager, looks_like_not_found
...
# If True:
#  - On startup and /reload_perms: Discord roles are authority (discord -> inquisitor)
#  - When /set_role is used: inquisitor updates the Discord role immediately (inquisitor -> discord)
sync_to_discord_roles: bool = True
...
# Map Inquisitor role names -> Discord role names (used when /set_role updates Discord)
INQ_ROLE_TO_DISCORD_ROLE = {
    "guest": "Guest",
    "member": "Member",  # choose the Discord role you actually use
    "mod": "Mod",
    "owner": "Admin",
}

# Higher wins if user has multiple mapped Discord roles
INQ_ROLE_RANK = {"Guest": 0, "Member": 1, "Mod": 2, "Owner": 3}
...
def _get_discord_role_by_name(guild: discord.Guild, role_name: str) -> discord.Role | None:
...
def _infer_inq_role_from_discord(member: discord.Member) -> str:
    """
    Discord is authority: choose the highest mapped role present on the member.
    If none mapped -> Guest.
    """
    best_role = "Guest"
    best_rank = INQ_ROLE_RANK.get(best_role, 0)

    for r in member.roles:
        if r.is_default():
            continue
        mapped = DISCORD_ROLE_TO_INQ_ROLE.get(r.name.strip().lower())
        if not mapped:
            continue
        rank = INQ_ROLE_RANK.get(mapped, 0)
        if rank > best_rank:
            best_role = mapped
            best_rank = rank
...
async def _sync_roles_from_discord(guild: discord.Guild) -> int:
    """
    Discord -> Inquisitor. Called on startup and /reload_perms.
    Also ensures users with no mapped role get the Discord Guest role (if it exists).
    Returns number of inquisitor role changes applied.
    """
    if not sync_to_discord_roles or not guild:
        return 0

    guest_discord_role = _get_discord_role_by_name(guild, INQ_ROLE_TO_DISCORD_ROLE.get("guest", "Guest"))
    desired_updates: dict[str, str] = {}
    to_add_guest: list[discord.Member] = []

    # Use cached members; ensure_all_members_have_records() already tried fetch_members
    for m in list(guild.members or []):
        if getattr(m, "bot", False):
            continue

        desired_inq = _infer_inq_role_from_discord(m)
        current_inq = PERMS.get_role(m.id)

        if desired_inq != current_inq:
            desired_updates[str(m.id)] = desired_inq

        # If user has NONE of the mapped Discord roles, enforce Discord Guest role (optional)
        has_any_mapped = any(
            (not r.is_default()) and (r.name.strip().lower() in DISCORD_ROLE_TO_INQ_ROLE)
            for r in m.roles
        )
        if not has_any_mapped and guest_discord_role and guest_discord_role not in m.roles:
            to_add_guest.append(m)

    # Apply inquisitor roles in one save
    changed = 0
    if desired_updates:
        changed = PERMS.set_roles_bulk(desired_updates, save=True)

    # Apply discord Guest role for unassigned members (does NOT touch overrides)
    if to_add_guest:
        try:
            for m in to_add_guest:
                await m.add_roles(guest_discord_role, reason="Inquisitor: default role assignment (Guest)")
        except Exception:
            log.exception("role-sync: failed adding Guest Discord role to some members")

    return changed


async def _set_discord_role_for_member(member: discord.Member, inq_role: str) -> None:
    """
    Inquisitor -> Discord. Called when /set_role is used.
    Removes other mapped Discord roles and adds the mapped one.
    """
    if not sync_to_discord_roles or not member or not member.guild:
        return

    role_key = str(inq_role).strip().lower()
    target_discord_name = INQ_ROLE_TO_DISCORD_ROLE.get(role_key)
    if not target_discord_name:
        return

    guild = member.guild
    target_role = _get_discord_role_by_name(guild, target_discord_name)
    if not target_role:
        log.warning("role-sync: target Discord role %r not found in guild", target_discord_name)
        return

    mapped_discord_names = set(DISCORD_ROLE_TO_INQ_ROLE.keys())

    to_remove = [
        r for r in member.roles
        if (not r.is_default())
        and (r.name.strip().lower() in mapped_discord_names)
        and (r.id != target_role.id)
    ]

    try:
        if to_remove:
            await member.remove_roles(*to_remove, reason="Inquisitor: role sync (set_role)")
        if target_role not in member.roles:
            await member.add_roles(target_role, reason="Inquisitor: role sync (set_role)")
    except Exception:
        log.exception("role-sync: failed to update discord roles for %s", member.id)
...
async def set_role_command(interaction: discord.Interaction, member: discord.Member, role: str):
    ...
    try:
        ...
        # NEW: if enabled, also sync the Discord role immediately
        if sync_to_discord_roles:
            await _set_discord_role_for_member(member, role)
...
async def reload_perms_command(interaction: discord.Interaction):
    ...
    if interaction.guild:
        ...
        if sync_to_discord_roles:
            applied = await _sync_roles_from_discord(interaction.guild)

    await interaction.followup.send(
        f"✅ Permissions reloaded. discord->inq role changes applied: {applied}",
        ephemeral=True
    )
...
def _cmdsig(cmd) -> str:
    # discord.py returns different types here:
    # - local commands may have .guild_ids
    # - fetched API AppCommand objects may have .guild_id
    scope = "GLOBAL"

    if hasattr(cmd, "guild_id") and getattr(cmd, "guild_id") is not None:
        scope = f"GUILD={getattr(cmd, 'guild_id')}"
    elif hasattr(cmd, "guild_ids"):
        gids = getattr(cmd, "guild_ids")
        scope = "GLOBAL" if gids is None else f"GUILDS={gids}"
...
async def on_ready():
    ...
    try:
        ...
        if g:
            ...
            if sync_to_discord_roles:
                applied = await _sync_roles_from_discord(g)
                log.info("role-sync: applied %s discord->inq role changes (startup)", applied)
