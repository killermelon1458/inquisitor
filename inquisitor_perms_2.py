...
class PermissionManager:
    ...
    async def ensure_all_members_have_records(self, guild) -> bool:
        ...
        for m in members:
            # ignore bots (keeps perms clean; can change later if you want bot accounts managed)
            ...
            # Pull human-readable names
            discord_name = getattr(m, "name", None)
            ...
            # Existing record: ensure keys exist + keep names fresh
            if not isinstance(u, dict):
                continue

            if u.get("discord_name") != discord_name:
                u["discord_name"] = discord_name
                changed = True
            if u.get("global_name") != global_name:
            ...
            if "discord_name" not in u:
                u["discord_name"] = discord_name
                changed = True
            if "global_name" not in u:
                u["global_name"] = global_name
                changed = True
    ...
    def set_roles_bulk(self, discord_id_to_role: Dict[str, str], *, save: bool = True) -> int:
        """
        Bulk update roles with a single rehydrate + optional single save.
        Returns number of users changed.
        """
        users = self.data.setdefault("users", {})
        changed = 0

        for did, role_name in (discord_id_to_role or {}).items():
            role_name_clean = str(role_name).strip().lower()
            if role_name_clean not in ROLE_CLASSES:
                raise ValueError(f"Unknown role: {role_name}")

            did_str = str(did)
            u = users.get(did_str) or {"role": "Guest", "overrides": {}, "usernames": [], "uuids": []}

            new_role = ROLE_CLASSES[role_name_clean]().role_name
